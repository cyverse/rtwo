...
try:
    ...
    def send_instance_email(*args, **kwargs):
        threepio.logger("send instance email. (%s) [%s]" % (args, kwargs))

try:
    from core.models.instance import update_instance_metadata
except:
    def update_instance_metadata(*args, **kwargs):
        threepio.logger("update instance metadata. (%s) [%s]" % (args, kwargs))
