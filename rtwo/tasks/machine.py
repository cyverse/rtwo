...
try:
    ...
    def send_image_request_email(*args, **kwargs):
        threepio.logger("send image request email. (%s) [%s]" % (args, kwargs))

try:
    from core.models.machine import createProviderMachine
except:
    def createProviderMachine(*args, **kwargs):
        threepio.logger("create ProviderMachine. (%s) [%s]" % (args, kwargs))

try:
    from core.models.machine_request import process_machine_request
except:
    def process_machine_request(*args, **kwargs):
        threepio.logger("process machine request. (%s) [%s]" % (args, kwargs))
...
def machine_migration_task(new_machine_name, old_machine_id,
        ...
        logger.debug("machine_migration_task task started at %s."
                     % datetime.now())
        if migrate_from == 'eucalyptus' and migrate_to == 'openstack':
            ...
            raise Exception("Cannot migrate from %s to %s" % (migrate_from,
                                                              migrate_to))
        logger.debug("machine_migration_task task finished at %s."
                     % datetime.now())
...
def select_and_build_image(machine_request, euca_imaging_creds,
    ...
    if old_provider == 'eucalyptus' and new_provider == 'eucalyptus':
        ...
        meta_name = '%s_%s_%s_%s' % ('admin',
                                     ...
                                     .replace(' ', '_').replace('/', '-'),
                                     machine_request
                                     .start_date.strftime('%m%d%Y_%H%M%S'))
        ...
        #                     manager.admin_driver._connection,
        #                     new_machine[0])
    return new_image_id
